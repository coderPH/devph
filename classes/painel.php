  ...
  // Sistema de carregarpagina dinamica
	...
	public static function inserir($array){//sempre q inserimos uma devida função $_POST passamos a ela e aqui retorna um array
		...
		if ($certo == true) {
			...
			$depoisId = Bd::conectar()->lastInsertId();
			$sql = Bd::conectar()->prepare("UPDATE `$nome_tabela` SET ordem_id = ? WHERE id = $depoisId");
			$sql->execute(array($depoisId));
		}
		
		return $certo;
	}
	// Sistema de atualizar
	public static function atualizar($array){//sempre q inserimos uma devida função $_POST passamos a ela e aqui retorna um array
			$certo = true;
			$inicio = false;
			$nome_tabela = $array['nome_tabela'];
		...
		if ($certo == true) {
			$parametros[] = $array['id'];
			$sql = Bd::conectar()->prepare($query.' WHERE id=?');
			$sql->execute($parametros);
			}
	...
	public static function selecionaTudo($tabela,$inicio = null,$fim = null){
		if($inicio == null && $fim == null)
			$sql = Bd::conectar()->prepare("SELECT * FROM `$tabela` ORDER BY ordem_id ASC");
		else
			$sql = Bd::conectar()->prepare("SELECT * FROM `$tabela` ORDER BY ordem_id ASC LIMIT $inicio,$fim");
			$sql->execute();
			return $sql->fetchAll();
	}
	...
	// Sistema para selecionar apenas um registro
	public static function selecionar($tabela,$query,$array){
		$sql = Bd::conectar()->prepare("SELECT * FROM `$tabela` WHERE $query");
		$sql->execute($array);
		return $sql->fetch();
	}
	// Sistema de ordernar itens
	public static function ordenarItens($tabela,$tipoOrdem,$idItem){
		if($tipoOrdem == 'cima'){
			$infoItemAtual = Painel::selecionar($tabela,'id=?',array($idItem));
			$ordem_id = $infoItemAtual['ordem_id'];
			$itemAtual = Bd::conectar()->prepare("SELECT * FROM `$tabela` WHERE ordem_id < $ordem_id ORDER BY ordem_id DESC LIMIT 1");
			$itemAtual->execute();
			if($itemAtual->rowCount() == 0)
				return false;
			$itemAtual = $itemAtual->fetch();
			Painel::atualizar(array('nome_tabela'=>$tabela,'id'=>$itemAtual['id'],'ordem_id'=>$infoItemAtual['ordem_id']));
			Painel::atualizar(array('nome_tabela'=>$tabela,'id'=>$infoItemAtual['id'],'ordem_id'=>$itemAtual['ordem_id']));
		}elseif ($tipoOrdem == 'baixo') {
			$infoItemAtual = Painel::selecionar($tabela,'id=?',array($idItem));
			$ordem_id = $infoItemAtual['ordem_id'];
			$itemAtual = Bd::conectar()->prepare("SELECT * FROM `$tabela` WHERE ordem_id > $ordem_id ORDER BY ordem_id ASC LIMIT 1");
			$itemAtual->execute();
			if($itemAtual->rowCount() == 0)
				return false;
			$itemAtual = $itemAtual->fetch();
